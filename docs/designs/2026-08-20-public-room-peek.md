...
- 有効な招待リンクからの案内
- 期限切れの `/share/:token` の 410（その URL では覗きも参加も不可）
- 非公開の未参加は 404
...
**設計意図:** 部屋の閲覧可否は `RoomPolicy`、招待リンクの有効期限は `ShareLinkPolicy`。410 は `/share/:token` だけ。公開部屋の一覧「覗く」→ `GET /rooms/:id` は別入口で `RoomPolicy` が効く。自動参加は見るとは入るを同じ操作にしているので外す。
...
- [ ] 未参加が期限切れの `/share/:token` を開くと 410。その URL 経由では覗きも参加もできない
- [ ] 非公開部屋の未参加は部屋・招待リンク・メンバー詳細・親タグ一覧が 404
- [ ] 非公開でも既存メンバーと作成者は部屋ページを見られる
- [ ] 公開部屋の一覧「覗く」→ `GET /rooms/:id` は `RoomPolicy` 判定。招待リンクの期限とは独立し、未参加でも見られる・参加できる
...
| 招待リンク | `ShareLink`。未参加が `/share/:token` を開くときだけ `ShareLinkPolicy` が効く |
...
| コード | 意味 | 入口 | 判定 |
|---|---|---|---|
| **410 Gone** | 招待リンクの有効期限切れ | `/share/:token` のみ | `ShareLinkPolicy#show?` |
| **404 Not Found** | 非公開部屋への未参加アクセス | 部屋・招待リンク・メンバー詳細・親タグ | `RoomPolicy#show?` など |

410 の説明に「公開なら一覧から覗ける」と書かない。期限切れの未参加は **その招待 URL では** 覗けず参加もできない。公開部屋を一覧の「覗く」→ `GET /rooms/:id` で見られるのは **別入口** で、`RoomPolicy` が公開設定だけを見るため、招待リンクの期限とは無関係。
...
| 状況 | 結果 |
|---|---|
| 未参加 × 期限切れの `/share/:token` | 410（その URL では覗きも参加も不可） |
| 未参加 × 非公開（部屋・リンク・メンバー詳細・親タグ） | 404 |
| 公開 × 未参加 × `GET /rooms/:id`（一覧「覗く」など。招待リンクの期限は見ない） | 200。参加レコードは増えない |
| 公開 × 未参加 × 有効な `/share/:token` からの案内 | 200。参加レコードは増えない |
| 既存メンバー / 作成者 | 200 |
